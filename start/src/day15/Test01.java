...
/*
 	파라미터로 전달된 문자열 
 		http://www.blackpink.org/member/@id=eun&pw=12345&tel=01031759042
 	중에서 넘겨진 데이터만 잘라서 출력하세요
 */
public class Test01 {

	public Test01() {
		String url = "http://www.blackpink.org/member/?id=eun&pw=12345&tel=01031759042";
		// 위 문자열에서 데이터부분만 추출하려면
		// ? 기호의 위치값을 알아낸 후 그 바로다음 위치에서부터 끝까지 잘라내면된다.
		int idx = url.indexOf('?');
		String param = url.substring(idx + 1);	// ==> "id=eun&pw=12345&tel=01031759042"
	...
	/*	int i = 0;
		...
		for(int i = 0;/* i < cnt*/token.hasMoreTokens(); i++) {
