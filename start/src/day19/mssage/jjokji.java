...
public class jjokji {
	...
	public jjokji() {
		...
		frame.setSize(400, 400); // 창 사이즈 설정
		frame.setVisible(true); // 기본은 안보이게 되어있다.
		frame.setResizable(false); // 창 사이즈 조절 안되게 하는 기능
		
		/*
		 	이제 프로그램이 시작 되었으므로
		 	네트워크 구성을 할 차례이다.
		 	여기서는 UDP통신을 해야 하므로
		 	DatagramSocket을 이용해서 네트워크 구성을 해야한다.
		  */
		try {
			sSocket = new DatagramSocket();
			rSocket = new DatagramSocket(100);
			/*
			...
			// 위 작업이 예외가 발생한 것은 이 프로그램이 더이상 제 기능을 할 수 없는 상태이므로 열려있는 소켓을 모두 닫아준다.
			close();
			e.printStackTrace();
		}
