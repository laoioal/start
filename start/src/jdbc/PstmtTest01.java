...
/*
 	이 클래스는
 	오라클에 있는 emp 테이블에서 부서번호가 ?? 번인 사원들의 정보를 조회해서 출력해주는 기능을 가진 클래스이다.
 	따라서 이 클래스는 객체가 되는 순간 데이터베이스에 연결할 준비가 되어있어야 한다.
 	==> 드라이버 로딩이 끝난 상태가 되어야 할것이다.
 */
public class PstmtTest01 {
	...
	public PstmtTest01() {
		...
		try {
			...
			// 3. 질의명령 준비하고
			//String sql = "SELECT empno eno, ename, job, mgr, TO_CHAR(hiredate, 'YYYY/MM/dd') hdate, deptno dno FROM emp ";
			...
			/*
			 	...
			 	==> 프로그램에서 JDBC로 명령을 전달하는 도구
			 		이떄 사용해야되는 질의명령은 완벽한 질의명령이어야 한다.
			...
			// 몇 사람이 조회될지 알 수 없으므로
			while(rs.next()) {
				// rs.next(0 ==> 작업 실행줄을 한줄 내려주는 함수. 이때 내린 결과가 꺼내올 데이터가 있으면 true를 반환해준다. 없으면 false 반환
				
				// 각 사원의 데이터를 꺼내고
				int eno = rs.getInt(1); // 컬럼이름 대신 위치값을 쓸수도 있다.
