...
//		
	...
	*/
	...
	// 직급을 입력받아서 해당 직급의 사원들의 정보를 조회주는 함수
...
//		System.out.println();
	...
	public void getDnoInfo(Scanner sc) {
		...
		try {
			...
			//		불완전한 질의명령이다.
			//		이때 사용하는 명령 전달도구는PreparedStatement를 사용한다.
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, no);
			// 질의명령 완성하고
			/*
			...
			rs = pstmt.executeQuery();
			while(rs.next()) { // 레코드포인터 한줄 내리고 ( EOF로 이동하면 false를 반환해준다.)
				// 데이터 추출하고
				int eno = rs.getInt("empno");
				String name = rs.getString("ename");
				String job = rs.getString("job");
				Date hdate = rs.getDate("hiredate");
				Date htime = rs.getDate("hiredate");
				int sal = rs.getInt("sal");
