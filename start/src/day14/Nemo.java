...
public class Nemo {
	...
	public Nemo() {
		...
		// 따라서 생성자를 정의를 할 경우는 내용이 이버있는 기본 생성자도 정의하는 것이 좋다.
	}
	...
	// 면적 계산해서 셋팅해주는 함수
	public void setArea() {
		// 이 함수를 호출하는 순간은 이미 가로와 세로 길이가 결정이 된 후 일 것이다.
		// 이미 필요한 데이터가 준비가 되어 있고 접근이 가능하므로
		// 따로 데이터를 전달받지 않아도 된다.
		area = width * height;
	}
	...
	public boolean equals(Object o) {
		...
		try {
			...
			// 만약 입력된 객체가 Nemo로 강제형변환 할때 예외가 발생하면
