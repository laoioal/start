...
public class Friend {
	/*
	 	...
	 	이때
	 	변수의 경우는 아무나 이 객체의 변수에 접근해서 데이터를 변경시켜버리면 안되므로
	 	변수는 감춰두고 변수에 접근할 수 있는 함수들을 만들어서 데이터를 처리한다.
	 */
	
	private String name, addr, tel, mail, birth;
	private int age;
	private char gne; // 성별을 기억할 변수로 남자는 'M', 여자는 'F'로 기억하리고 하자.
	...
	public void setName(String name) {
		this.name = name;
		/*
		 	this : 현재 실행중인 객체자기자신의 주소를 기억하는 변수
		 	
		 		이 클래스가 객체가 여러개가 만들어 질 수 있는데
