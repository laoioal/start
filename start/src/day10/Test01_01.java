...
//		num1 = t1.no1;	// no1의 접근지정자는 Test01 클래스 내에서만 사용할 수 있도록 private로 지정되어있다.
		num2 = t1.no2;	// Test01 클래스와 같은 패키지(폴더)에 있으므로 접근지정자가 생략된 경우 이상은 모두
		num3 = t1.no3;	// 같은 패키지 내이기 때문에 사용 가능하다.
		num4 = t1.no4;	// no4의 접근지정자가 모든 곳에서 접근 가능하도록 public 으로 지정되어있다.
